...
base_paths <- list(
  min = file.path("data", "la_atmin"),
  max = file.path("data", "la_atmax"),
  rng = file.path("processed", "la_dtrng")
)
...
for (year in yearly_data) {
  ...
  for (i in 1:length(min_files)) { 
    ...
    filename <- sub("ATMIN", "DTRNG", basename(min_file))
    filepath <- file.path(rng_path, filename)
    
    # calculate and save avg raster
    min_raster <- raster(min_file)
    max_raster <- raster(max_file)
    avg_raster <- (max_raster - min_raster)
...
for (year in names(yearly_data)) {
  ...
  rng_files <- list.files(year_paths[["RNG"]])
  num_files <- min(length(min_files), length(max_files))
  actual_files <- length(rng_files)
...
min_test <- raster(file.path("data", "la_atmin", "LA_ATMIN_2004", "LA_ATMIN_2004_DOY001.tif"))
max_test <- raster(file.path("data", "la_atmax", "LA_ATMAX_2004", "LA_ATMAX_2004_DOY001.tif"))
rng_test <- raster(file.path("processed", "la_dtrng", "LA_DTRNG_2004", "LA_DTRNG_2004_DOY001.tif"))

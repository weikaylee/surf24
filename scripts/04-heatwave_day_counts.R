...
# consts
years <- c(2003:2020)
input_path <- file.path("data", "la_atmin")
output_path <- file.path("processed", "heatwave_counts")
# shp_file <- file.path("data", "la_county_boundary", "County_Boundary.shp")
shp_file <- file.path("data", "la_census_shp", "LA_HRS_income.shp")
threshold_file <- file.path("data", "summer_85p.tif")
...
for (i in sequence) {
  year <- years[i]
  plot_raster_path <- file.path("processed", "heatwave_counts", paste0("LA_HEATWAVE_CNT_", year, ".tif"))

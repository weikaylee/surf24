...
source(file.path("scripts", "07-clean_zipcode_data.R"))
message("Getting data to collapse...")
source(file.path("scripts", "08-load_all_processed_data.R"))
    ...
    for (file_name in names(layer_names)) {
      if (grepl(file_name, file, fixed=TRUE)) {
        ...
        if (!file_name %in% not_bins) { 
          layer_name <- paste(layer_names[[file_name]], bin_labels[[file_name]], sep="_")
        }
        return(layer_name)
      }
    }
...
main <- function(var_data_by_year, zipcodes_processed, zipcode_colnames, layer_colnames, weights, output_path) {
  ...
  # for (year in 2003:2004) {
    ...
    for (i in 1:nrow(zipcodes_processed)) {
      ...
      extracted_vals <- terra::extract(yearly_rasters, zipcode_geo, ID=FALSE, touches=TRUE)
...
bin_labels <- list(
  DAILY_TEMP_BINS_AVG = c("Inf_0", paste0(seq(0, 39), "_", seq(1, 40)), "40_Inf"),
  DAILY_TEMP_BINS_MAX = c("Inf_0", paste0(seq(0, 39), "_", seq(1, 40)), "40_Inf"),
  DAILY_TEMP_BINS_MIN = c("Inf_0", paste0(seq(0, 39), "_", seq(1, 40)), "40_Inf"),
  DTRNG_BINS = c("Inf_0", paste0(seq(0, 39), "_", seq(1, 40)), "40_Inf"),
  MAX_RNG = c("Inf_0", paste0(seq(0, 4), "_", seq(1, 5)), "5_Inf")
)
...
# save final output!
csv_file_path <- file.path(output_path, paste0("LA_zipcode_measures_all_years_max_rng", ".csv"))
...
# library(sf)
# library(sp)
# collapsed_measures_df <- read.csv(file.path("processed", "collapsed_measures", "LA_zipcode_measures_all_years.csv"))

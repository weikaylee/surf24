...
# load er data
er_path <- file.path("processed", "health_data", "er_health_data_counts.csv")
er_df <- read.csv(er_path)


# load zipcode data 
zipcodes_path <- file.path("processed", "collapsed_measures", "zipcodes_processed.shp")
zipcodes <- read_sf(zipcodes_path)


# load zipcode population data 
pop_path <- file.path("data", "pca", "ACSDP5Y2022.DP05-Data.csv")
...
# save final er data
save_path <- file.path("processed", "health_data", "er_rates_catchment.csv")
write.csv(new_rates_df_sorted, save_path, row.names = FALSE)


# plot some data across zipcodes, across years 
# create spatialdf
df <- read.csv(file.path("processed", "health_data", "er_rates_catchment.csv")) %>% data.frame
...
plot_list <- lapply(names(df_list), function(year) {
  ggplot(df_list[[year]]) + 
    geom_sf(aes(geometry=geometry, fill=EDvisits)) + 
    labs(title=paste(year, "ED visits per 100,000 population"), fill="ED visits") + 
...
store <- ggarrange(plotlist=plot_list, nrow=4, ncol=4, common.legend = TRUE, legend="right") # if nothing shows up, call "store" in console
ggsave(filename=file.path("figures", "catchment_er_final.png"), height=10, width=15, store)

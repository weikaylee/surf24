...
# load mortality data
mortality_path <- file.path("processed", "health_data", "mortality_counts.csv")
...
# save final df!
save_path <- file.path("processed", "health_data", "mortality_rates_catchment.csv")
write.csv(catchment_mort_df, save_path, row.names = FALSE)


# plot some data across zipcodes, across years
# create spatialdf
df <- read.csv(save_path) %>% data.frame
...
plot_list <- lapply(names(df_list), function(year) {
  ggplot(df_list[[year]]) + 
    ...
    labs(title=paste(year, "Mortality Rate"), fill="Mortality rate") + 
...
ggsave(filename=file.path("figures", "catchment_mortality_final.png"), height=10, width=15, store)
